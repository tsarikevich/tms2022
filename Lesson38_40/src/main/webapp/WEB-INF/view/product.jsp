...
<c:if test="${not empty oneProduct}">
    <div class="container-fluid">
        ...
        <c:forEach items="${oneProductImages}" var="image">
            <c:if test="${oneProduct.getId()==image.getProductId()}">
                <img class="mx-auto d-block" style="height:auto; max-width: 400px;"
                     src="${contextPath}/images/${image.getImagePath()}"
                     alt="${image.getImagePath()}">
            </c:if>
        </c:forEach>
        <br>
        <div class="container">
        ...
        <a style="text-decoration: none"
           href="${contextPath}/cart/add?productId=${oneProduct.getId()}">
